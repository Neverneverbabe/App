...
let themeToggleBtn, filterButton, body, sidebarToggleButton, sidebarToggleIcon, currentTabNameDisplay,
    ...
    filterModal, filterModalCloseButton, filterOptionsItemsContainerModal, filterTypeOptionsContainer,
    filterApplyBtnModal, filterClearBtnModal;
...
window.onload = async () => {
    ...
    filterTypeOptionsContainer = document.getElementById('filter-type-options-container');
        ...
        try {
            switch (activeTabId) {
                case 'watch-now-tab':
                    ...
                    break;
                case 'explore-tab':
                    await ContentManager.populateExploreTab(currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, SeenItemsManager.isItemSeen);
                    break;
                case 'library-tab':
                    await LibraryManager.populateLibraryTab(SeenItemsManager.isItemSeen, isLightMode, onCardClick);
                    break;
                case 'seen-tab':
                    SeenItemsManager.populateSeenTab(currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick);
                    break;
                case 'search-tab':
                    SearchManager.populateSearchTab(currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, SeenItemsManager.isItemSeen);
                    break;
                default:
    ...
    function renderTypeOptions(container, selectedType) {
        container.innerHTML = MEDIA_TYPE_OPTIONS.map(opt =>
            `<div class="dropdown-item filter-type-option" data-type="${opt.value}">${opt.label} <span class="checkmark">✔</span></div>`
        ).join('');

        container.querySelectorAll('.filter-type-option').forEach(item => {
    ...
    function openFilterModal() {
        ...
        tempSelectedType = currentTypeFilter || '';
        renderTypeOptions(filterTypeOptionsContainer, tempSelectedType);
    ...
    // Click handler for individual age rating filter options within the modal
    ...
    // Click handler for type filter options
    filterTypeOptionsContainer.addEventListener('click', (event) => {
        event.stopPropagation();
        const target = event.target.closest('.filter-type-option');
        if (!target || target.dataset.type === undefined) return;
        tempSelectedType = target.dataset.type;
        renderTypeOptions(filterTypeOptionsContainer, tempSelectedType);
    });
    ...
    filterApplyBtnModal.addEventListener('click', (event) => {
        ...
        currentTypeFilter = tempSelectedType;
        closeFilterModal();
        filterButton.classList.toggle('filter-active', currentAgeRatingFilter.length > 0 || currentTypeFilter !== '');
    ...
    window.addEventListener('scroll', () => {
        ...
        if ((window.innerHeight + window.scrollY) >= document.documentElement.scrollHeight - 300) {
            ContentManager.loadMoreExploreItems(currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, SeenItemsManager.isItemSeen);
        }
    });
    ...
    SearchManager.setupSearchListeners(searchInput, currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, SeenItemsManager.isItemSeen);
};

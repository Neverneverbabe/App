 ...
 */
export async function performSearch(query, reRenderOnly, currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, isItemSeenFn) {
    ...
    if (reRenderOnly && cachedSearchResults.length > 0) {
        ...
        if (currentTypeFilter) {
            filteredResults = filteredResults.filter(item => item.media_type === currentTypeFilter);
        }
        ...
        try {
            ...
            const fetchType = currentTypeFilter || 'multi';
            const results = await fetchSearchResults(query, fetchType);
 ...
 * @param {function} isItemSeenFn - Function to check if an item is seen.
 */
export function setupSearchListeners(searchInput, currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, isItemSeenFn) {
    // Debounce search input to avoid excessive API calls
    searchInput.addEventListener('input', () => {
        clearTimeout(searchTimer);
        searchTimer = setTimeout(() => {
            performSearch(searchInput.value.trim(), false, currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, isItemSeenFn);
 ...
 * @param {function} isItemSeenFn - Function to check if an item is seen.
 */
export function populateSearchTab(currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, isItemSeenFn) {
    const searchInput = document.getElementById('search-input');
    if (searchInput.value.trim().length >= 3 && cachedSearchResults.length > 0) {
        performSearch(searchInput.value.trim(), true, currentAgeRatingFilter, currentTypeFilter, isLightMode, onCardClick, isItemSeenFn);
    } else {
        const searchResultsContainer = document.getElementById('search-results-container');
